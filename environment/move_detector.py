...
def get_move_info(move, wild_card_of_game=1):
    move_size = len(move)
    wild_rank = EnvCard2Rank[wild_card_of_game]
    ...
    if move_size == 1:
        if EnvCard2Rank[move[0]] == wild_rank:
            return {'type': TYPE_1_SINGLE, 'rank': 15}
        else:
            return {'type': TYPE_1_SINGLE, 'rank': EnvCard2Rank[move[0]]}

    if move_size == 2:
        if EnvCard2Rank[move[0]] == EnvCard2Rank[move[1]] and EnvCard2Rank[move[0]] != 16 and \
                EnvCard2Rank[move[0]] != 17:
            if EnvCard2Rank[move[0]] == wild_rank:
                return {'type': TYPE_2_PAIR, 'rank': 15}
            else:
    ...
    if move_size == 3:
        if number_of_wild_cards <= 2 and len(move_dict_without_wildcard) == 1 and \
                list(move_dict_without_wildcard.keys())[0] != 16 and list(move_dict_without_wildcard.keys())[0] != 17:
            if list(move_dict_without_wildcard.keys())[0] == wild_rank:
                return {'type': TYPE_3_TRIPLE, 'rank': 15}
            else:
    ...
    if move_size == 4:
        if len(move_dict[16]) == 2 and len(move_dict[17]) == 2:
            return {'type': TYPE_14_JOKER_BOMB, 'rank': 16}
        if number_of_wild_cards <= 2 and len(move_dict_without_wildcard) == 1 and \
                list(move_dict_without_wildcard.keys())[0] != 16 and list(move_dict_without_wildcard.keys())[0] != 17:
            if list(move_dict_without_wildcard.keys())[0] == wild_rank:
                return {'type': TYPE_8_BOMB_4, 'rank': 15}
            else:
    ...
    if move_size == 5:
        if number_of_wild_cards <= 2 and len(move_dict_without_wildcard) == 1:
            if list(move_dict_without_wildcard.keys())[0] == wild_rank:
                return {'type': TYPE_9_BOMB_5, 'rank': 15}
            else:
        ...
        if len(move_dict_without_wildcard) == 2 and sorted(rank2number.values()) == [2, 3]:
            ...
            if rank == wild_rank:
                return {'type': TYPE_4_3_2, 'rank': 15}
            else:
        ...
        if number_of_wild_cards == 1 and len(
            ...
            if (16 in number2rank[2]) or (17 in number2rank[2]):
                rank = [rank for rank in number2rank[2] if rank != 16 and rank != 17][0]
                if rank is None:
                    return {'type': TYPE_15_WRONG, 'rank': None}
                elif rank == wild_rank:
                    return {'type': TYPE_4_3_2, 'rank': 15}
                ...
                if wild_rank in list(rank2number.keys()):
                    return {'type': TYPE_4_3_2, 'rank': 15}
                else:
        ...
        if number_of_wild_cards == 2 and len(move_dict_without_wildcard) == 2:
            if 16 in list(rank2number.keys()) or 17 in list(rank2number.keys()):
                rank = [rank for rank in list(rank2number.keys()) if rank != 16 and rank != 17][0]
                if rank == wild_rank:
                    return {'type': TYPE_4_3_2, 'rank': 15}
                else:
    ...
    if move_size == 6:
        if number_of_wild_cards <= 2 and len(move_dict_without_wildcard) == 1:
            if EnvCard2Rank[move_without_wildcard[0]] == wild_rank:
                return {'type': TYPE_11_BOMB_6, 'rank': 15}
            else:
        ...
        if number_of_wild_cards == 0:  # check the normal case of 6 cards while wild card number is 0
            if 2 in number2rank and len(number2rank[2]) == 3 and sorted(number2rank.values())[0][0] + 2 == \
                    sorted(number2rank.values())[0][1] + 1 == sorted(number2rank.values())[0][2] \
    ...
    if move_size == 7:
        if number_of_wild_cards <= 2 and len(move_dict_without_wildcard) == 1:
            if EnvCard2Rank[move_without_wildcard[0]] == wild_rank:
                return {'type': TYPE_12_BOMB_7, 'rank': 15}
            else:
                return {'type': TYPE_12_BOMB_7, 'rank': EnvCard2Rank[move_without_wildcard[0]]}
    if move_size == 8:
        if number_of_wild_cards <= 2 and len(move_dict_without_wildcard) == 1:
            if EnvCard2Rank[move_without_wildcard[0]] == wild_rank:
                return {'type': TYPE_13_BOMB_8, 'rank': 15}
            else:
